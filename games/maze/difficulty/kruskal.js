...
document.addEventListener("keydown", function(event) {
  //console.log(event);
  if (
    (event.which == 38 && !player.walls[0]) ||
    (event.which == 87 && !player.walls[0])
  ) {
    player = grid[index(player.i, player.j - 1)];
  } else if (
    (event.which == 39 && !player.walls[1]) ||
    (event.which == 68 && !player.walls[1])
  ) {
    player = grid[index(player.i + 1, player.j)];
  } else if (
    (event.which == 40 && !player.walls[2]) ||
    (event.which == 83 && !player.walls[2])
  ) {
    player = grid[index(player.i, player.j + 1)];
  } else if (
    (event.which == 37 && !player.walls[3]) ||
    (event.which == 65 && !player.walls[3])
  ) {
    player = grid[index(player.i - 1, player.j)];
  }
...
reset = function() {
  var goalReachedMazeSound = new Audio(
    "../../../soundeffects/goalReachedMaze.mp3"
  );
