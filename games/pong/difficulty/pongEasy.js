...
class Pong {
  ...
  collide(player, ball) {
    ...
    if (
      ...
      if (
        this.ball.position.y > this.players[i].position.y &&
        ball.velocity.y < 400
      ) {
        ball.velocity.y +=
          400 *
          ((ball.position.y - this.players[i].position.y) / (padelHeight / 2));
      } else if (
        this.ball.position.y < this.players[i].position.y &&
        ball.velocity.y > -400
      ) {
        ...
        ball.velocity.y = ball.velocity.y + 300 * (Math.random() - 0.5);
        //ball.velocity.y += 300 * (Math.random() - .5);
      }

      if (ball.velocity.length < this._canvas.width * 1.7) {
  ...
  drawScore() {
    ...
    this.players.forEach((player, index) => {
      ...
      const offset =
        align * (index + 1) - (CHAR_W * chars.length) / 2 + this.CHAR_PIXEL / 2;
      chars.forEach((char, position) => {
        this._context.drawImage(
          this.CHARS[char | 0],
          offset + position * CHAR_W,
          20
        );
      });
    });
  ...
  start() {
    if (this.ball.velocity.x === 0 && this.ball.velocity.y === 0) {
      this.ball.velocity.x = 400 * (Math.random() > 0.5 ? 1 : -1);
  ...
  update(dt) {
    ...
    if (
      this.ball.top < 0 ||
      (this.ball.bottom > this._canvas.width && bounceTimer)
    ) {
    ...
    if (
      this.ball.position.x > this._canvas.width / 6 &&
      this.ball.position.x < this._canvas.width * 0.583
    ) {
      if (
        this.players[1].position.y <
        this.ball.position.y - this._canvas.width * 0.0166
      ) {
    ...
    // Ball very close to AI puck
      ...
      if (
        this.players[1].position.y <
        this.ball.position.y - padelHeight * 0.4
      ) {
        ...
        this.players[1].position.y +=
          0.75 * (this.ball.position.y - this.players[1].position.y);
      }
    }
